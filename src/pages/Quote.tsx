...
import { ArrowLeft, Quote as QuoteIcon, Tag, BookOpen, Calendar, Play, Pause, Square, Loader } from 'lucide-react';
...
export default function Quote() {
  ...
  const { state, toggle, stop } = useNarration(slug || '');
    ...
    <div style={{ background: 'var(--bg-main)', minHeight: '100vh' }}>
      ...
      <header className="sticky top-0 z-50 border-b" style={{ borderColor: 'var(--border)', background: 'rgba(10,10,15,0.95)', backdropFilter: 'blur(12px)' }}>
        <div className="max-w-3xl mx-auto px-6 h-14 flex items-center gap-4">
          ...
          {/* Compact narration in header */}
          <div className="ml-auto flex items-center gap-2">
            {isActive && (
              <button onClick={stop} className="w-7 h-7 rounded-lg flex items-center justify-center transition-all hover:opacity-70"
                style={{ background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
                <Square className="w-3 h-3" fill="currentColor" style={{ color: 'var(--text-muted)' }} />
              </button>
            )}
            <button
              onClick={toggle}
              ...
              style={{
                background: isActive ? `${quote.accent}20` : 'var(--bg-card)',
                border: `1px solid ${isActive ? quote.accent + '50' : 'var(--border)'}`,
                color: isActive ? quote.accent : 'var(--text-secondary)',
              }}
            >
              {state === 'loading' ? (
                <><motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1, ease: 'linear' }}><Loader className="w-3 h-3" /></motion.div><span>Loading</span></>
              ) : state === 'playing' ? (
                <>
                  <motion.div className="flex items-center gap-0.5" animate={{ opacity: [1, 0.5, 1] }} transition={{ repeat: Infinity, duration: 0.8 }}>
                    {[3, 5, 3].map((h, i) => <span key={i} className="w-0.5 rounded-full" style={{ height: h, background: quote.accent }} />)}
                  </motion.div>
                  <span>Pause</span>
                </>
              ) : state === 'paused' ? (
                <><Play className="w-3 h-3" fill="currentColor" /><span>Resume</span></>
              ) : (
                <><Play className="w-3 h-3" fill="currentColor" /><span>Listen</span></>
              )}
            </button>
          </div>
        </div>
      ...
      <div className="max-w-3xl mx-auto px-6 py-16">
        ...
        {/* Main Quote */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="relative mb-10">
          <QuoteIcon className="absolute -top-4 -left-2 w-12 h-12 opacity-10" style={{ color: quote.accent }} />
          <blockquote className="text-3xl md:text-4xl font-bold leading-tight pl-6"
        ...
        {/* Big Listen Button */}
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="flex items-center gap-3 mb-12 pl-6">
          <button
            onClick={toggle}
            className="flex items-center gap-3 px-6 py-3 rounded-xl font-semibold text-sm transition-all hover:scale-105 active:scale-95"
            style={{
              background: isActive ? `${quote.accent}20` : `linear-gradient(135deg, ${quote.accent}30, ${quote.accent}15)`,
              border: `1px solid ${quote.accent}40`,
              color: quote.accent,
            }}
          >
            {state === 'loading' ? (
              <><motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1, ease: 'linear' }}><Loader className="w-4 h-4" /></motion.div> Loading audio...</>
            ) : state === 'playing' ? (
              <>
                <motion.div className="flex items-center gap-0.5">
                  {[...Array(4)].map((_, i) => (
                    <motion.span key={i} className="w-0.5 rounded-full inline-block"
                      ...
                      animate={{ height: ['12px', `${16 + i * 4}px`, '8px', '14px'] }}
                      transition={{ repeat: Infinity, duration: 0.6, delay: i * 0.1 }} />
                  ))}
                </motion.div>
                <Pause className="w-4 h-4" fill="currentColor" /> Pause narration
              </>
            ) : state === 'paused' ? (
          ...
          {isActive && (
            <button onClick={stop} className="flex items-center gap-2 px-4 py-3 rounded-xl text-sm transition-all hover:opacity-70"
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}
          className="rounded-xl p-6 mb-10" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="mb-10">
          <h3 className="text-xs font-bold uppercase tracking-widest mb-3" style={{ color: 'var(--text-muted)' }}>Why It Matters</h3>
          <p className="text-base leading-relaxed" style={{ color: 'var(--text-secondary)' }}>{quote.context}</p>
        </motion.div>
        ...
        <div className="grid grid-cols-2 gap-4 pt-8" style={{ borderTop: '1px solid var(--border)' }}>
          {prev ? (
            <button onClick={() => { stop(); navigate(`/quote/${prev.slug}`); }}
          ...
          {next ? (
            <button onClick={() => { stop(); navigate(`/quote/${next.slug}`); }}
